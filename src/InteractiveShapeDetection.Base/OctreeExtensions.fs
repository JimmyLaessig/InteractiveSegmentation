...
module OctreeExtensions =
    ...
    module Octree =
        ...
        let choose (decisionFun : OctreeNode -> GridCell -> int[] -> Option<'T>*bool) (tree: Octree): 'T[] = 
            ...
            let rec traverse (node : OctreeNode) (cell: GridCell) (path : int[])=
                       
                match node with
                | Empty             -> ()
                | Leaf _            -> 
                                let t,b = decisionFun node cell path
                ...
                | Node (_,children) ->   
                                ...
                                match t,b with
                                | None, false   -> ()
                                | None, true    -> node.Children    |> Array.iteri(fun i child -> traverse (child.Value) (cell.GetChild i) (Array.append path [|i|]))                               
                                | Some t, false -> nodes    |> CSet.add t |> ignore
                                | Some t, true  -> node.Children    |> Array.iteri(fun i child -> traverse (child.Value) (cell.GetChild i) (Array.append path [|i|]))
