...
module LodProgress =
    ...
    type GeometryRef = { node : LodData; geometry : IndexedGeometry; range : Range1i } with
...
module Sg = 
    type PointCloud(data : ILodProvider, config : PointCloudInfo, progress : Progress) =
        interface ISg
    ...
    let pointCloud (data : ILodProvider) (info : PointCloudInfo) =
        PointCloud(data, info, Progress.empty) :> ISg

    let pointCloud' (data : ILodProvider) (info : PointCloudInfo) (progress : Progress) =
        PointCloud(data, info, progress) :> ISg


                

namespace InteractiveShapeDetection.Visualization.Semantics
...
module PointCloudRenderObjectSemantics = 
    ...
    type PointCloudHandler(node : InteractiveShapeDetection.Visualization.Sg.PointCloud, view : IMod<Trafo3d>, proj : IMod<Trafo3d>, viewportSize : IMod<V2i>, progress : InteractiveShapeDetection.Visualization.LodProgress.Progress, runtime : IRuntime) =
        let queueCount = 8
        ...
        let pool = Aardvark.Base.Rendering.GeometryPool.createAsync runtime
        let calls = Aardvark.Base.Rendering.DrawCallSet(true)
        let mutable activeSize = 0L


        let pruning = false
        let inactive = ConcurrentHashQueue<InteractiveShapeDetection.Visualization.LodProgress.GeometryRef>()
        let mutable inactiveSize = 0L

        let workingSet = CSet.empty


        let activate (n : InteractiveShapeDetection.Visualization.LodProgress.GeometryRef) =
        ...
        let deactivate (n : InteractiveShapeDetection.Visualization.LodProgress.GeometryRef) =
        ...
        member x.Activate() =
            ...
            let effect (ct : CancellationToken) (n : LodData) =
                stepwise {
                    let data =  
                        try 
                    ...
                    match data with
                        | Some (Some v) ->
                            ...
                            let gref : InteractiveShapeDetection.Visualization.LodProgress.GeometryRef= { geometry = v; range = range; node = n }
        ...
        member x.DrawCallInfos =
            calls |> Aardvark.Base.Rendering.DrawCallSet.toMod


    [<Semantic>]
    type PointCloudSemantics() =
        member x.RenderObjects(l : InteractiveShapeDetection.Visualization.Sg.PointCloud) =
            ...
            match l.Config.boundingBoxSurface with
                ...
                | Some surf ->
                    let trafos = 
                        h.WorkingSet 
                            |> ASet.toMod 
                            |> Mod.map ( fun a -> a |> Seq.toArray |> Array.map ( fun v -> v.bounds.UnitBoxTrafo))
                    

                    let iSg = Box3d.Unit
                                |> IndexedGeometry.ofBox3d
